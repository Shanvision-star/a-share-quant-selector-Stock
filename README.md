...
> **当前开发主线：`web` 分支。** 后续功能、修复、回测模块和实盘执行规划都以 `web` 分支为准；其他旧分支仅作为历史参考。

> **当前优先级：先完成回测模块，再推进模拟/人工执行，最后预留量化实盘接口。** QMT / miniQMT 和自动买卖暂不作为 20 万资金阶段的主线能力。

> **分支规则：代码和 Markdown 文档改动都先开 `codex/*` 或明确 feature 分支处理，验证后再合并回 `web`，避免半成品污染主线。**
...
- **回测优先规划**：回测模块先升级为可复用交易引擎；20 万资金阶段优先模拟盘、人工确认和手动下单，QMT / miniQMT 只保留适配接口。
...
- 📝 **Markdown 复盘库** — 侧边栏“复盘库”按日期维护每日一篇 Markdown，支持截图上传、拖入或粘贴，标题候选、本地回退、状态筛选和全文搜索；可从股票详情和跟踪运营将股票加入今日复盘

### 回测优先与实盘执行预留规划

- **回测模块定位** — 当前回测用于验证策略信号，后续会拆成 `DataPortal / SignalSource / ExecutionEngine / Portfolio / Analyzer`，让日线回测、分钟线回测、模拟盘和实盘共用同一套买卖规则。
- **当前开发顺序** — 第一阶段先完整化回测引擎，包括交易日历、A 股撮合规则、组合资金曲线、回测持久化和前端回测详情；第二阶段再做模拟盘/人工确认；第三阶段才评估券商 API。
- **分时买点确认** — 日线策略只生成候选池，盘中再用 1 分钟线、VWAP、分时均线、量能、回撤和涨跌停状态确认是否生成 `OrderIntent`。
- **当前资金方案** — 银河证券 QMT 量化权限按 300 万门槛预留；当前 20 万资金阶段不把 QMT 自动下单作为主线，优先 `paper` 模拟、`confirm_manual` 人工确认和成交流水复盘。
- **QMT 预留接口** — 保留 `QmtBrokerAdapter` 设计和 `OrderIntent` 表结构，未来资金或券商权限满足时再从只读模式接入。
- **实盘风控底线** — 默认不开自动实盘；必须配置单票最大金额、单日最大买入金额、最大持仓数、停牌/ST/涨跌停禁买和一键停止开关。
- **低 token 成本原则** — 大模型不进入实时交易循环，只读取结构化摘要做复盘和说明；策略、回测、分时触发、人工确认和未来券商适配全部由确定性 Python 代码执行。
...
启动工作台后，从侧边栏进入“复盘库”（`/review-library`）：选择日期创建当天复盘，填写标题、状态、标签、重点股票和 Markdown 正文；截图可通过“添加截图”上传、拖入或直接粘贴，并会插入可预览的 Markdown 图片引用。标题候选会优先使用已配置的标题服务，不可用时保留本地回退；左侧列表支持按标题、正文和股票全文搜索，并可按状态筛选。

股票详情页和跟踪运营页都提供“加入今日复盘”操作，重复加入同一股票不会写入两次。复盘文件和截图附件都保存在 `data/review_library/`：每篇文件位于 `YYYY/MM/YYYY-MM-DD.md`，截图位于同级 `YYYY-MM-DD.assets/` 目录。备份或迁移整个 `data/review_library/` 目录即可保留全部复盘和附件。
...
│  - /api/config          策略参数读写(revision) │
│  - /api/strategy/*      策略结果/缓存状态    │
│  - /api/update          数据更新（SSE流）     │
│  - /api/data/init       首次全量初始化（SSE） │
│  - /api/txt/*           通达信TXT导出        │
└─────────────────────────────────────────────┘
```
...
- `data/`：运行数据目录：
  ...
  - `review_library/`：日期 Markdown 复盘库；复盘文件按 `YYYY/MM/YYYY-MM-DD.md` 保存，截图附件位于同级 `YYYY-MM-DD.assets/` 目录。备份整个目录即可迁移复盘记录和附件；
...
| **参数设置** | `/settings` | 草稿编辑+手动保存（离开未保存提醒），保存后即时生效 |
| **系统状态** | `/status` | 只读检查后端、数据新鲜度、策略缓存、更新作业、Tracking 与集成配置健康 |
| **回测** | `/backtest` | 历史日期回溯扫描，验证策略参数有效性 |
| **复盘库** | `/review-library` | 每日 Markdown 复盘、截图附件、标题候选、全文搜索与状态筛选；股票详情和跟踪运营可加入今日复盘 |

### 参数保存与前端加载优化（2026-04）

- 参数接口采用 revision 机制：`GET /api/config` 返回 `revision`，`POST /api/config` 需携带 `expected_revision`；版本冲突返回 `409`，前端提示刷新后重试。
- 设置页采用草稿状态：切换页面前如果存在未保存改动会弹出确认，不再因路由切换直接丢失参数编辑内容。
- 首页与策略结果页新增请求去重/取消能力，减少并发请求回写旧数据导致的闪烁与等待。
- K 线请求支持 `AbortSignal`，切换股票/周期时会取消过期请求，降低图表区无效渲染。
...
| **SSE实时进度流** | 统一作业包含预检查、首次初始化、快慢路径分流日更、策略重建，所有阶段通过 Server-Sent Events 实时推送到前端 |

...
> **当前开发主线：`web` 分支。** 后续功能、修复、回测模块、QMT / miniQMT 量化执行规划都以 `web` 分支为准；其他旧分支仅作为历史参考。
...
- **回测与QMT规划**：回测模块将升级为可复用交易引擎，并按银河 QMT / miniQMT 设计模拟盘、手动确认和小资金自动执行链路。
...
- **银河 QMT 适配** — 项目以已开通的银河证券 QMT 为目标接口；若实际客户端为 miniQMT，只要可用 `xtquant`，仍通过同一个 `QmtBrokerAdapter` 接入。
- **实盘风控底线** — 默认不开自动实盘；必须配置单票最大金额、单日最大买入金额、最大持仓数、停牌/ST/涨跌停禁买和一键停止开关。
- **低 token 成本原则** — 大模型不进入实时交易循环，只读取结构化摘要做复盘和说明；策略、回测、分时触发和 QMT 下单全部由确定性 Python 代码执行。

详细计划见：[QMT 回测与实盘量化执行落地计划](docs/QMT/qmt_backtest_live_execution_plan.md)。

...
async def trigger_update(
    ...
    strategies: str = Query(None, description="按策略筛选；逗号分隔，如 b1,b2,bowl,brick,zettaranc；为空或 all 表示全部"),
):
    """触发数据更新（SSE 流式返回进度）
    ...
    strategies 为逗号分隔的策略集合（b1/b2/bowl/brick/zettaranc），为空或包含 all 时按全部执行。
    """

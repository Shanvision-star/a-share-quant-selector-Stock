...
# --改动点：
    1.加入选股结果txt文本输出导入通达信自选股，方便进行最后的选股；
    2.加入企业钉钉机器人结果输出，提供TXT输出结果到钉钉软件的功能；
# --目标：
    1.编写B2、B3选股程序，进行B1、B2、B3策略的分类输出；
    2.加入搬砖策略，对短线日K反转点的抓取，优先做短线趋势线之上的反转(删掉kdj值的影响)，次选黄线以上白线以下kdj结合策略的日线反转
    3.根据日 k 线选股策略，隔日根据最终选股结果进行买入，跟踪买入。尾盘跟踪当日持仓k线收盘价2.50分-2.57分，对比前一天执行的选股策略和止损  
    卖出点，执行卖出或者是持有策略；
# --回测功能完善：
    1.回测30日当日买入、卖出跟踪B1、B2、B3胜率
    2.回测B2买入，量化交易的胜率
以下内容基于原https://github.com/Dzy-HW-XD/a-share-quant-selector项目中Redme内容，对项目添加txt文件导出，程序云服务器定时执行时间修改注释；
...
# 4. 首次全量抓取数据
python3 main.py init(注意cd 目录下运行，定时执行策略同理)

# 5. 执行选股（自动更新数据、选股、发送钉钉）
python3 main.py run(注意cd 目录下运行，定时执行策略同理)

# 6. 快速测试（只处理前500只股票）
python3 main.py run --max-stocks 500(注意cd 目录下运行，定时执行策略同理)

# 7. 启动Web界面
python3 main.py web(注意cd 目录下运行，定时执行策略同理)
...
#### 选股条件
# 备注：后续优化非空头与多头区间提醒，防止遗漏知行短期趋势线 ≥知行多空线，处于击穿对手盘的股票
...
```
项目根目录/
├── main.py                   # 主程序入口：负责组织整个选股、导出、通知流程
├── web_server.py             # Web 服务入口：提供网页查看、查询、展示等功能
├── B1_PATTERN_MATCH.md       # B1 完美图形匹配说明文档

├── strategy/                 # 策略模块：负责选股逻辑、图形匹配、信号计算
│   ├── __init__.py           # 包初始化文件
│   ├── base_strategy.py      # 策略基类：定义统一策略接口
│   ├── bowl_rebound.py       # 碗口反弹策略实现
│   ├── strategy_registry.py  # 策略注册器：统一管理可用策略
│   ├── pattern_config.py     # B1 完美图形配置
│   ├── pattern_library.py    # B1 图形案例库管理
│   ├── pattern_matcher.py    # 图形相似度匹配引擎
│   └── pattern_feature_extractor.py  # 图形特征提取模块

├── utils/                    # 工具模块：通用辅助功能
│   ├── akshare_fetcher.py    # 数据获取：从 akshare 拉取股票数据
│   ├── csv_manager.py        # CSV 数据读写管理
│   ├── technical.py          # 技术指标计算：KDJ、EMA、MA 等
│   ├── kline_chart.py        # K线图生成（标准版）
│   ├── kline_chart_fast.py   # K线图生成（快速版）
│   ├── dingtalk_notifier.py  # 钉钉消息通知
│   └── tdx_exporter.py       # 通达信导出器：生成可导入的股票 txt 文件

├── config/                   # 配置目录：放运行参数和环境配置
│   ├── config.yaml           # 主配置文件
│   ├── strategy_params.yaml  # 策略参数配置
│   └── github.yaml           # GitHub 相关配置

├── web/                      # Web 前端资源
│   ├── templates/            # HTML 模板
│   └── static/               # 静态资源：CSS、JS、图片等

├── data/                     # 原始或中间股票数据目录（自动创建）
│   └── ...                   # 各股票 CSV 数据文件

└── output/                   # 程序运行输出目录
    ├── tdx/                  # 通达信导入文件输出目录
    │   ├── tdx_stocks_latest.txt      # 最新一次执行生成的通达信股票文件
    │   └── tdx_stocks_YYYYMMDD_HHMMSS.txt  # 按时间归档的历史导出文件
    ├── charts/               # 生成的K线图、形态图等
    └── reports/              # 分析报表、结果汇总等
```
...
**作者GitHub源代码库**: https://github.com/Dzy-HW-XD/a-share-quant-selector
...
基于 Python + akshare 的A股量化选股系统，支持以下功能：

- **量化策略**：实现碗口反弹策略，支持K线图可视化、Web管理界面和钉钉自动通知。
- **自动过滤风险股**：自动跳过 ST/*ST、退市、异常股票。
- **B1完美图形匹配**：基于历史成功案例的三维相似度匹配排序（双线+量比+形态）。
- **B2三分类策略**：横盘突破、灾后重建、平行重炮三类拆分检测逻辑。
...
- 🧱 **B2三分类策略** - 已按横盘突破、灾后重建、平行重炮三类拆分检测逻辑，便于继续补案例和独立调参
...
- ⚙️ **自动并发/单核降级** - `run` 与 `run --b1-match` 默认按 CPU 自动分配线程；检测到单核设备时会自动切回单线程，避免线程切换拖慢执行
...
- `web/`：Web 前端资源目录，包含 Flask 的模板与静态文件：
  - `static/`：前端静态资源（JS、CSS、图片等）；
  - `templates/`：Flask 模板（HTML 页面），用于渲染系统概览、选股结果与图表。

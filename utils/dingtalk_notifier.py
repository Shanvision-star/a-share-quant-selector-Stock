...
"""
    ...
    def send_backtest_results_with_charts(
        self,
        results: list,
        stock_names=None,
        stock_data_dict=None,
        ...
        """
        if not results:
            self.send_text("🔍 回溯扫描完成：无符合条件股票。")
            return False

        if stock_names is None:
            stock_names = {}
        if params is None:
            params = {'lookback_days': 3, 'k_threshold': 20, 'max_drop_pct': 5}
        ...
        summary = [
            ...
            "",
            f"时间: {now}",
            f"条件: 最近 {params.get('lookback_days', 3)} 天 K < {params.get('k_threshold', 20)} 且 不破短期趋势线 {params.get('max_drop_pct', 5)}%",
            "━" * 30,
            "",
            f"共扫描结果: {len(results)} 只符合条件",
            ""
        ]
        ...
        for item in results:
            code = item.get('code')
            name = item.get('name', stock_names.get(code, '未知'))
            content_lines = [
                f"### {code} {name}",
                f"最后日期: {item.get('last_date')}",
                f"收盘价: {item.get('last_close')}",
                f"最近 {params.get('lookback_days', 3)} 天 K 值: {item.get('k_values')}",
                f"最小 K 值: {item.get('min_k')}",
                f"与短期趋势线偏离(%): {item.get('deviation_pct')}",
                f"最大回落: {item.get('max_drawdown_pct')}%",
                ""
            ]
            self.send_markdown(f"{code} {name} 回溯结果", "\n".join(content_lines))

            if stock_data_dict and code in stock_data_dict:
                df = stock_data_dict[code]
                try:
                    chart_path = generate_kline_chart(
                        stock_code=code,
                        ...
                        category='backtrack',
                        params=params,
                        key_candle_dates=item.get('key_dates', []),
                        output_dir=str(temp_dir),
                        show_text=False,
                        show_legend=True
                    )
                    self.send_image(chart_path, f"{code} K线图")
                except Exception as e:
                    print(f"✗ 生成 {code} 的回溯K线图失败: {e}")
                    continue
    ...
    def send_b2_match_results(self, results: list) -> None:
        ...
        lines += [
            ...
            "**案例来源**: 星环科技(688031) 2025-12-05 标准B2形态",
        ]
    ...
    def send_b2_match_results_with_charts(
        ...
        summary_lines = [
            ...
            f"案例库: 星环科技(688031) 2025-12-05 标准B2形态",
            "━" * 30,
            "",
        ]
        for i, r in enumerate(results, 1):

...
- `POST /api/update`
  - 触发统一更新作业（SSE）。
  - 先做数据完整性预检查（`precheck`），本地无CSV时会先执行全量初始化（`init_full`），随后再进入日更。
  - 支持 `allow_intraday_fast`（盘中手动启用快路径）与 `init_if_empty`（首次自动初始化）参数。
- `GET /api/data/status`
  - 读取当前数据是否新鲜。
- `POST /api/data/init`
  - 单独触发首次全量初始化（6年历史，SSE进度）。

它的业务定位很明确：

- 这个页面关注“先确保数据完整，再执行日更分流”。
- 首次使用与日常增量更新走同一套可观测SSE进度体系。
...
- `run_data_update()`
  ...
  - 更新前会先发送 `precheck` 阶段，必要时先跑 `init_full`。
  - 通过 `ThreadPoolExecutor` 避免阻塞事件循环。
- `run_data_init()`
  - 专门服务首次初始化场景，输出初始化实时进度。

这段逻辑的业务含义是：

- Web 接口要保留“可流式更新进度”的体验。
- 更新流程变为“预检查 -> 快慢路径分流日更 -> 慢路径补齐”，首次场景自动切换到全量初始化。
- 底层抓数逻辑仍然是同步的，因此继续通过线程池桥接。

...
| X2 | 连续 2 日收盘破 BBI | 离线脚本 `simulate_trade()` 内部计数，连续两日收盘低于当日 BBI 后按第二日收盘价离场 |
| X3 | 时间止损 N 日未盈利 | `hold_days_limit` 参数（v1 默认 20，超期且回撤>5%出场） |
| X4 | 止盈 | `take_profit_pct=15` 触发 |

> 当前离线脚本已启用 X1+X2+X3+X4；后续若接入通用 `backtest_engine`，仍需单独评估引擎执行器是否支持 X2 这种状态型出场。
...
| 引擎执行器对状态型出场支持不足 | 离线脚本已覆盖 X1/X2/X3/X4；若接入通用引擎，需要补执行器级 X2 规则或保持离线脚本为权威验证口径 |
